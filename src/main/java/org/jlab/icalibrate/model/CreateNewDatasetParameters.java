...
public class CreateNewDatasetParameters {
     ...
     * number. This means not all steps will be the same size if the goal is to include the min
     * value and the max value exactly. In particular the current algorithm assumes the last step is
     * of varying size.
     *
    ...
    public void checkOptionValidity() throws ValidationException {
        ...
        if (minAttenuator > maxAttenuator) {
            throw new ValidationException(
                    "Min Attenuator can not be greater than Max Attenuator");
        }
        ...
        if (computeAttenuatorRange() < numberOfSteps) {
            throw new ValidationException(
                    "Number of Attenuator steps is greater than attenuator range");
        }
