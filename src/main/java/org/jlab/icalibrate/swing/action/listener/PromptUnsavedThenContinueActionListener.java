...
public final class PromptUnsavedThenContinueActionListener implements ActionListener {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(PromptUnsavedThenContinueActionListener.class.getName());
   ...
   */
  public PromptUnsavedThenContinueActionListener(
      ICalibrateFrame frame, ActionListener continueAction) {
    this.frame = frame;
  ...
  public void actionPerformed(ActionEvent e) {
    if (frame.isUnsavedData()) {
      ...
      doNotSaveButton.addActionListener(
          new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
              SwingUtilities.getWindowAncestor(cancelButton).dispose();
      ...
      // JOptionPane will create a modal dialog that pauses code execution here, yet magically does
      // not freeze EDT
      int response =
      ...
      if (response == 0) { // Save
        SaveHCDThenContinueActionListener saveAction =
            new SaveHCDThenContinueActionListener(frame, continueAction);
        saveAction.actionPerformed(new ActionEvent(this, ActionEvent.ACTION_PERFORMED, "Continue"));
      }
    } else {
      doContinueAction();
    }
