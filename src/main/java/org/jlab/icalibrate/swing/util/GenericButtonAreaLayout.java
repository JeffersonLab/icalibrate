 ...
 * 
 * This class is essentially a copy and paste of
 * javax.swing.plaf.basic.BasicOptionPaneUI.ButtonAreaLayout, but with the 
 * private constructor made public so button sizing could be managed.
 */
    ...
    public void layoutContainer(Container container) {
        ...
        if (children != null && children.length > 0) {
            ...
            for (int counter = 0; counter < numChildren; counter++) {
                ...
                if (getSyncAllWidths()) {
                    children[index].setBounds(x, insets.top + topMargin,
                            maxWidth, maxHeight);
                } else {
                    children[index].setBounds(x, insets.top + topMargin, pref.width,
                            pref.height);
                }
                if (xOffset != 0) {
                    x += xOffset;
