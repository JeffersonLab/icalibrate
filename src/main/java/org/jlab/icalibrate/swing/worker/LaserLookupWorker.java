...
public class LaserLookupWorker extends MinimumExecutionSwingWorker<Laser, Void> {
  ...
  protected Laser doWithMinimumExecution() throws Exception {
    ...
    try {
      ...
      throw new AppException(
          "Unable to perform EPICS CA Get of PV: " + laserAPv + " - " + e.getMessage(), e);
    } catch (TimeoutException e) {
      throw new AppException("Timeout waiting for EPICS CA Get of PV: " + laserAPv, e);
    }
    ...
    if (total > 1) {
      throw new AppException("More than one laser assigned to hall " + hall.name());
    }
