...
/**
 * This is a SwingWorker which looks up the target in use for the selected hall. The hall target PVs
 * are consulted to determine the target. Once the worker is complete it decrements the count down
 * latch such that the parent thread can determine when all concurrently executing child threads are
...
public class TargetLookupWorker extends MinimumExecutionSwingWorker<String, Void> {
  ...
  protected String doWithMinimumExecution() throws Exception {
    ...
    String targetPv = ICalibrateApp.APP_PROPERTIES.getProperty(hallPrefix + "TARGET_PV");
    ...
    try {
      ...
      throw new AppException(
          "Unable to perform EPICS CA Get of PV: " + targetPv + " - " + e.getMessage(), e);
    } catch (TimeoutException e) {
      throw new AppException("Timeout waiting for EPICS CA Get of PV: " + targetPv, e);
    }
