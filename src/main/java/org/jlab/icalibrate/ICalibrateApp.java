...
/**
 * The entry point of the ion chamber calibration application. The application measures dose rates
 * of ion chambers at experimental hall end stations at Jefferson Lab and uses this information to
 * set dose limits to protect the machine during production runs.
 *
 * <p>The ICalibrateApp class loads the configuration properties, initializes a network connection
 * to EPICS, and initializes the Swing GUI.
 *
 * @author ryans
 */
public class ICalibrateApp {

  private static final Logger LOGGER = Logger.getLogger(ICalibrateApp.class.getName());
      ...
      if (file != null) {
        try {
          ...
          throw new InitializationException(
              "Unable to load file "
                  + file.getName()
                  + ": Ion Chambers do not match current configuration",
              e);
        }
      }
